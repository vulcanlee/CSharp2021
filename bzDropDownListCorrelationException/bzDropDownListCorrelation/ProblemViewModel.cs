...
namespace bzDropDownListCorrelation
{
    public class ProblemViewModel
    {
        ...
        public async Task ProblemViewModelInit()
        {
            ...
            #region 使用 先 await 再使用 AddRange ，似乎沒有問題
            ...
            #region 先對 DataSource 做操作，再使用 await 等待，最後使用 AddRange ，似乎有問題
            //MyClassList.Clear();
            //MyClassList.Insert(0, new MyClass { Name = "請選擇", Id = -1 });
            //QueryCondition.MyId = -1;
            //var myLists = await MyClassService.Get();
            ...
            #region 另外一種可行做法，先 await，再同步進行下拉選單的清單項目設定(沒有使用 AddRange)
            var myLists = await MyClassService.Get();
            MyClassList.Clear();
            MyClassList = myLists;
            MyClassList.Insert(0, new MyClass { Name = "請選擇", Id = -1 });
        ...
        public async Task MasterListChanged(Syncfusion.Blazor.DropDowns.ChangeEventArgs<int, MyClass> args)
        {
            ...
            #region 使用這個方式，第一次會有連動，之後切換我的下拉選單項目，就會造成無法連動顯示
            ...
            #region 使用背景工作來進行相關清單更新，這樣也會失敗，注意全部清單項目數量數字會有更新
